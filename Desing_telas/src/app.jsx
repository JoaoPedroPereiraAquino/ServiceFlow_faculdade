...
function App() {
  ...
  const renderScreen = () => {
    ...
    switch (screen) {
      ...
      case 'notifications': return <NotificationsScreen {...shared} />;
  ...
  const themed = React.useMemo(() => ({
    ...
    _logoVariant: tweaks.logoVariant,
  }), [theme, radiusScale, tweaks.density, tweaks.logoVariant]);

  const screenLabel = {
    login: '01 Login', signup: '02 Cadastro', dashboard: '03 Dashboard',
    os: '04 Nova OS', client: '05 Novo Cliente', oslist: '06 Lista de OS',
    notifications: '07 Notificações', profile: '08 Perfil',
  }[top.screen] || 'Screen';

  // Bottom nav only on top-level app screens
  const TOP_LEVEL = ['dashboard', 'oslist', 'notifications', 'profile'];
  const showBottomNav = TOP_LEVEL.includes(top.screen);
  const bottomNavEl = showBottomNav ? (
    <BottomNav theme={themed} current={top.screen}
      onChange={(k) => setStack([{ screen: k, params: {} }])}
      onNewOS={() => nav.push('os')}
    />
  ) : null;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ position: 'relative', width: PHONE_WIDTH, height: PHONE_HEIGHT }}>
          <DeviceFrame theme={themed} bottomNav={bottomNavEl}>
            {renderScreen()}
          </DeviceFrame>
